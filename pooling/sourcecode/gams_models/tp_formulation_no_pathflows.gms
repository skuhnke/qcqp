...
* General Constraints
	...
	GE_FL_MAX_WD(WD) ..				SUM(WS, FL(WS, WD)) + SUM((WS, PL), FL(WS, PL) * PR(PL, WD)) 	=L=		FL_MAX_UN(WD);
	GE_FL_MAX_PI(WS, UN_IN) ..		FL(WS, UN_IN)											=L=		FL_MAX(WS, UN_IN);
	GE_FL_MAX_PI_PR(PL, WD) ..		SUM(WS, FL(WS, PL) * PR(PL, WD))								=L=		FL_MAX(PL, WD);

* TP-Formulation	
	TP_PR_BALANCE(PL) ..		SUM(WD, PR(PL, WD))				=E=		ONE;
	TP_PO_MIN_WD(WD, CO) ..		SUM(WS, PO_WS(WS, CO) * FL(WS, WD)) + SUM((WS, PL), PO_WS(WS, CO) * FL(WS, PL) * PR(PL, WD))
								=G=		PO_MIN_WD(WD, CO) * (SUM(WS, FL(WS, WD)) + SUM((WS, PL), FL(WS, PL) * PR(PL, WD)));
	TP_PO_MAX_WD(WD, CO) .. 	SUM(WS, PO_WS(WS, CO) * FL(WS, WD)) + SUM((WS, PL), PO_WS(WS, CO) * FL(WS, PL) * PR(PL, WD))
								=L=		PO_MAX_WD(WD, CO) * (SUM(WS, FL(WS, WD)) + SUM((WS, PL), FL(WS, PL) * PR(PL, WD)));
	TP_PO_MIN_WD_PP(WD, CO) ..	IS_ACTIVE_MIN(WD, CO) * (SUM(WS, PO_WS(WS, CO) * FL(WS, WD)) + SUM((WS, PL), PO_WS(WS, CO) * FL(WS, PL) * PR(PL, WD)))
								=G=		IS_ACTIVE_MIN(WD, CO) * PO_MIN_WD(WD, CO) * (SUM(WS, FL(WS, WD)) + SUM((WS, PL), FL(WS, PL) * PR(PL, WD)));
	TP_PO_MAX_WD_PP(WD, CO) .. 	IS_ACTIVE_MAX(WD, CO) * (SUM(WS, PO_WS(WS, CO) * FL(WS, WD)) + SUM((WS, PL), PO_WS(WS, CO) * FL(WS, PL) * PR(PL, WD)))
								=L=		IS_ACTIVE_MAX(WD, CO) * PO_MAX_WD(WD, CO) * (SUM(WS, FL(WS, WD)) + SUM((WS, PL), FL(WS, PL) * PR(PL, WD)));								
	
	TP_VALID_1(WS, PL) ..		SUM(WD, FL(WS, PL) * PR(PL, WD))		=E=		FL(WS, PL);
	TP_VALID_2(PL, WD) ..		SUM(WS, FL(WS, PL) * PR(PL, WD))		=L=		FL_MAX_UN(PL) * PR(PL, WD); 

* Objective Function
	OBJECTIVE .. 			OBJ 	=E=	 	SUM((WS, WD), COST(WS, WD) * FL(WS, WD)) + 
											SUM((WS, PL, WD), (COST(WS, PL) + COST(PL, WD)) * FL(WS, PL) * PR(PL, WD));
